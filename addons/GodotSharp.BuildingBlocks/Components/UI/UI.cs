...
namespace GodotSharp.BuildingBlocks
{
    public static class UI
    {
        ...
        public static Label Label(string prop)
            => Label($"{prop}Label", prop.Capitalize());
        ...
        public static RichTextLabel RichTextLabel(string name, string text = "", bool bbcode = true, bool selectable = false)
        {
            var label = new RichTextLabel { Name = name, Text = text, BbcodeEnabled = bbcode, FitContent = true, ScrollActive = false, ScrollFollowing = true, AutowrapMode = AutowrapMode.Off, ContextMenuEnabled = selectable, MetaUnderlined = true, HintUnderlined = true, SelectionEnabled = selectable };
            label.MetaClicked += x => OS.ShellOpen((string)x);
            return label;
        }
        ...
        public static OptionButton OptionButton(string name, params string[] options)
        {
            ...
            void SetOptions()
            {
                for (var idx = 0; idx < options.Length; ++idx)
                {
                    ...
                    edit.AddItem(option.Capitalize());
        ...
        public static Separator HSep(string name = "Sep")
            => new HSeparator { Name = name };

        public static Separator VSep(string name = "Sep")
            => new VSeparator { Name = name };
        ...
        public static void SetMenuItems(this PopupMenu source, params string[] menuItems)
        {
            ...
            static string GetTitle(string item)
                => (item.EndsWith("...") ? item : item.Split('.', 2).First()).Capitalize();
        }

        public static void SetMenuItems(this PopupMenu source, IDictionary<string, string> menuItems)
        {
            source.Clear();
            ...
            {
                ++idx;
                source.AddItem(kvp.Key);
                source.SetItemMetadata(idx, kvp.Value);
            }
        }
